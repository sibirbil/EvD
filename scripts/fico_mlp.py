...
traj_x_thetastar_preds = jnp.argmax(traj_x_thetastar_probs, axis = -1)
...
traj_x_noised_preds = jnp.argmax(traj_x_noised_logits, axis = -1)
...
generated_df = pd.DataFrame(jnp.column_stack([traj_x_thetastar_preds, inverted]), columns = df.columns)
...
test_scores = jax.vmap(score, in_axes = (0, None, None))(noised_params, X_test, test_preds)
